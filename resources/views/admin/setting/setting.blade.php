	...
	<div class="row col-sm-6">
		...
		<div class="col-sm">
			<select name="akun[]" id="supp select2" class="form-control" required width="100%">
				<option value="">Pilih Akun</option> @foreach ($akun as $akn)
				<option value="{{ $akn->no_akun }}">{{ $akn->no_akun }} - {{ $akn->nm_akun }} </option> @endforeach </select>
		</div>
	</div> @endforeach
	<input type="submit" class="btn btn-primary btn-send" value=" Update Setting "> </form> @endsection